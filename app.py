...
def main():
    ...
    if page == "Homepage":
        ...
        sns.pairplot(
            df,
            vars=["magnesium", "flavanoids", "nonflavanoid_phenols", "proline"],
            hue="alcohol",
        )
        st.pyplot()
    else:
        ...
        row_number = st.number_input(
            "Select row", min_value=0, max_value=len(df) - 1, value=0
        )
        st.markdown("#### Predicted")
        st.text(
            model.predict(
                df.drop(["alcohol"], axis=1).loc[row_number].values.reshape(1, -1)
            )[0]
        )
